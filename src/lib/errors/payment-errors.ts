...
export const ERROR_RECOVERY_STRATEGIES: Record<PaymentErrorType, PaymentError> =
  {
    ...
    [PaymentErrorType.INSUFFICIENT_FUNDS]: {
      ...
      userMessage:
        'Insufficient funds. Please use a different card or add funds to your account.',
      severity: ErrorSeverity.MEDIUM,
      retryable: true,
    ...
    [PaymentErrorType.INVALID_EXPIRY_MONTH]: {
      ...
      userMessage:
        "Your card's expiration month is invalid. Please check and try again.",
      severity: ErrorSeverity.LOW,
      retryable: false,
    ...
    [PaymentErrorType.INVALID_EXPIRY_YEAR]: {
      ...
      userMessage:
        "Your card's expiration year is invalid. Please check and try again.",
      severity: ErrorSeverity.LOW,
      retryable: false,
      suggestedAction: 'CORRECT_EXPIRY',
      metadata: {
    ...
    [PaymentErrorType.TAX_CALCULATION_ERROR]: {
      ...
      userMessage:
        'Unable to calculate taxes for your location. Please try again or contact support.',
      severity: ErrorSeverity.MEDIUM,
      retryable: true,
      retryDelay: 1000,
      maxRetries: 2,
    ...
    [PaymentErrorType.INVOICE_ERROR]: {
      ...
      userMessage:
        "There was an error processing your invoice. We'll try again shortly.",
      severity: ErrorSeverity.HIGH,
      retryable: true,
      retryDelay: 10000,
      maxRetries: 3,
...
export function calculateRetryDelay(
  ...
  let delay =
    config.baseDelayMs * Math.pow(config.backoffMultiplier, attempt - 2);
...
export async function retryPaymentIntentConfirmation(
  ...
  return retryWithBackoff(async () => {
    ...
    throw new Error(
      `Payment intent in unexpected state: ${paymentIntent.status}`
    );
  }, PaymentErrorType.PROCESSING_ERROR);
}
...
export function formatErrorForUser(error: PaymentError): {
  ...
  return {
    ...
    showAlternativePayment:
      config.metadata?.showAlternativePaymentMethods || false,
  };
}
...
export function requiresUserAction(error: PaymentError): boolean {
  return (
    error.metadata?.requiresUserAction === true ||
