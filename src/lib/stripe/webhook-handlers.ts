      ...
      if (item.product.type === 'DIGITAL' && item.product.downloadUrl) {
        await tx.download.create({
          data: {
            ...
            secureUrl: await generateSecureDownloadUrl(item.product.downloadUrl, order.userId!),
  ...
  await db.$transaction(async (tx) => {
    ...
    await tx.subscription.update({
      ...
      data: {
        ...
        price: subscription.items.data[0].price.unit_amount || existingSubscription.price,
...
async function handleInvoicePaymentSucceeded(invoice: Stripe.Invoice): Promise<void> {
  if (invoice.subscription) {
    ...
    console.log(`Invoice payment succeeded for subscription: ${subscription.stripeSubscriptionId}`);
  }
}
...
async function handleInvoicePaymentFailed(invoice: Stripe.Invoice): Promise<void> {
  if (invoice.subscription) {
    ...
    console.log(`Invoice payment failed for subscription: ${subscription.stripeSubscriptionId}`);
  }
}
...
async function handleChargeRefunded(charge: Stripe.Charge): Promise<void> {
  ...
  console.log(`Refund processed for order: ${payment.order.orderNumber} ($${refundAmount / 100} CAD)`);
}
