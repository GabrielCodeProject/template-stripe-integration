...
async function checkRedis(): Promise<HealthCheck> {
  ...
  try {
    ...
    return {
      service: 'redis',
      status: responseTime < 50 && pong === 'PONG' && testValue === 'test' ? 'healthy' : 'degraded',
      responseTime,
      details: {
        ping: pong,
    ...
    (async (): Promise<HealthCheck> => {
      ...
      try {
        ...
        await new Promise((resolve, reject) => {
          dns.resolve4('stripe.com', (err: any, addresses: string[]) => {
            if (err) reject(err)
...
export async function GET() {
  ...
  try {
    ...
    const [dbCheck, redisCheck, stripeCheck, externalChecks] = await Promise.all([
      checkDatabase(),
      checkRedis(),
      checkStripe(),
    ...
    if (overallStatus !== 'healthy') {
      console.warn('Health check issues detected:', {
        status: overallStatus,
        unhealthyServices: allChecks.filter(c => c.status === 'unhealthy').map(c => c.service),
        degradedServices: allChecks.filter(c => c.status === 'degraded').map(c => c.service)
      })
    }
